...
#
#   cp legacy/novel-agent/agent-document/docs/deploy/docker/.env.split.example legacy/novel-agent/agent-document/docs/deploy/docker/.env.split
#   # 编辑 WORKER_HOST / WORKER_SSH
#   bash legacy/novel-agent/agent-document/docs/deploy/scripts/setup-split-config.sh
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEPLOY_DIR="$(cd "$SCRIPT_DIR/../docker" && pwd)"
NOVEL_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
REPO_ROOT="$(cd "$NOVEL_ROOT/.." && pwd)"
...
_CLI_DEPLOY_ENV="${DEPLOY_ENV:-}"
_CLI_SPRING_PROFILE="${SPRING_PROFILES_ACTIVE:-}"
_CLI_NACOS_NS="${NACOS_NAMESPACE:-}"
...
# dev / prod 命名空间与 Spring Profile 对齐（避免 prod 机误连 dev 配置）
DEPLOY_ENV="${_CLI_DEPLOY_ENV:-${DEPLOY_ENV:-prod}}"
if [[ -n "$_CLI_NACOS_NS" ]]; then
  NACOS_NAMESPACE="$_CLI_NACOS_NS"
elif [[ "$DEPLOY_ENV" == "prod" ]]; then
  NACOS_NAMESPACE="${NACOS_NAMESPACE_PROD:-${NACOS_NAMESPACE:-}}"
else
  NACOS_NAMESPACE="${NACOS_NAMESPACE_DEV:-${NACOS_NAMESPACE:-}}"
fi
if [[ -n "$_CLI_SPRING_PROFILE" ]]; then
  SPRING_PROFILES_ACTIVE="$_CLI_SPRING_PROFILE"
else
  SPRING_PROFILES_ACTIVE="${SPRING_PROFILES_ACTIVE:-$DEPLOY_ENV}"
fi
export NACOS_NAMESPACE SPRING_PROFILES_ACTIVE DEPLOY_ENV

if [[ -z "${NACOS_NAMESPACE:-}" ]]; then
  echo "[split-setup] ERROR: NACOS_NAMESPACE 为空，请在 .env.split 设置 NACOS_NAMESPACE_PROD / NACOS_NAMESPACE_DEV"
  exit 1
fi
...
MW_WG_IP="${MW_WG_IP:-10.66.0.2}"
WORKER_WG_IP="${WORKER_WG_IP:-10.66.0.3}"
GATEWAY_UPSTREAM="${GATEWAY_UPSTREAM:-http://${MW_HOST}:8080}"

echo "[split-setup] MW=$MW_HOST (wg=$MW_WG_IP)  Worker=$WORKER_HOST (wg=$WORKER_WG_IP)  env=$DEPLOY_ENV  nacos=$NACOS_NAMESPACE  profile=$SPRING_PROFILES_ACTIVE"
...
JWT_SECRET=${JWT_SECRET:?请在 .env.split 设置 JWT_SECRET（≥32 字符）}
AGENT_PYTHON_BASE_URL=http://python-lb:8000
AGENT_CONTENT_BASE_URL=http://${WORKER_WG_IP}:8091
JAVA_OPTS_CONSUMER=${JAVA_OPTS_CONSUMER:--Xms64m -Xmx220m -XX:MaxMetaspaceSize=96m -XX:+UseSerialGC -XX:TieredStopAtLevel=1 -XX:+ExitOnOutOfMemoryError -Dfile.encoding=UTF-8}
...
JAVA_MEM_LIMIT_CONSUMER=${JAVA_MEM_LIMIT_CONSUMER:-320m}
...
HOST_IP=${WORKER_WG_IP}
MW_HOST=${MW_HOST}
WORKER_HOST=${WORKER_HOST}
NACOS_SERVER_ADDR=${MW_WG_IP}:8848
NACOS_USERNAME=${NACOS_USERNAME}
NACOS_PASSWORD=${NACOS_PASSWORD}
NACOS_NAMESPACE=${NACOS_NAMESPACE}
...
BILLING_BASE_URL=http://${MW_WG_IP}:8092
CRAWL_FETCH_CONCURRENCY=${CRAWL_FETCH_CONCURRENCY:-2}
CRAWL_BROWSER_CONCURRENCY=${CRAWL_BROWSER_CONCURRENCY:-1}
SPRING_DATASOURCE_URL=jdbc:postgresql://${MW_WG_IP}:5432/novel_agent?sslmode=disable
SPRING_DATASOURCE_USERNAME=postgres
SPRING_DATASOURCE_PASSWORD=${SPRING_DATASOURCE_PASSWORD}
SPRING_DATA_REDIS_HOST=${MW_WG_IP}
SPRING_DATA_REDIS_PASSWORD=${SPRING_DATA_REDIS_PASSWORD}
SPRING_RABBITMQ_HOST=${MW_WG_IP}
SPRING_RABBITMQ_USERNAME=novel_agent
...
python "$NOVEL_ROOT/scripts/publish_nacos_config.py"
