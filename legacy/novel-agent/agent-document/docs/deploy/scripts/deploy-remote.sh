...
#
# 用法（Git Bash）：
#   bash legacy/novel-agent/agent-document/docs/deploy/scripts/deploy-remote.sh
set -euo pipefail
...
COMPOSE_FILE="legacy/novel-agent/agent-document/docs/deploy/docker/docker-compose.yml"
ENV_REL="legacy/novel-agent/agent-document/docs/deploy/docker/.env"
