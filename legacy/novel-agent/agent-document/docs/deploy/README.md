...
> ⚠️ **本文档随 `novel-agent/` 旧微服务栈一并归档至 `legacy/`，已停止维护。** 生产部署权威指南已迁移至 `novel-studio/deploy/README.md`；速查见 `.cursor/rules/deploy-ops.mdc`。本文保留作历史参考，**勿据以部署**。
>
> 注意：本目录下 `ci/build-python-ai.sh`、`docker/Dockerfile.python-ai`、`docker/.env.*` 仍被活跃 CI 引用（详见 `legacy/README.md`），勿删。
...
- Secrets 清单：`legacy/novel-agent/agent-document/docs/deploy/ci/SECRETS.example.md`
- CI 脚本目录：`legacy/novel-agent/agent-document/docs/deploy/ci/`

## 前置条件

1. 服务器已 clone 仓库至 `/opt/novel-agent`（或自定义 `MW_REMOTE_DIR` / `WORKER_REMOTE_DIR`）
2. 配置 split 环境：`legacy/novel-agent/agent-document/docs/deploy/docker/.env.split`（勿提交）
...
```bash
bash legacy/novel-agent/agent-document/docs/deploy/ci/build-java.sh agent-gateway
bash legacy/novel-agent/agent-document/docs/deploy/ci/deploy-java.sh gateway mw
```
...
- 告警规则模板：`legacy/novel-agent/agent-document/docs/deploy/observability/prometheus-alerts.yml`
...
bash legacy/novel-agent/agent-document/docs/deploy/scripts/restore-postgres.sh /opt/novel-agent/backups/pg/novel_agent-YYYY-MM-DD-HHMM.sql.gz
```

MW 建议 cron：`0 3 * * * /opt/novel-agent/legacy/novel-agent/agent-document/docs/deploy/scripts/backup-postgres.sh`
...
```bash
bash legacy/novel-agent/agent-document/docs/deploy/scripts/deploy-route-v2.sh all
...
- 发布脚本：`python legacy/novel-agent/scripts/publish_nacos_config.py`
