...
> ⚠️ **历史文档（旧微服务栈本地启动流程），已随 `novel-agent/` 归档至 `legacy/`，停止维护。** 本地开发请改用 `frontend && npm run dev`；全栈验收走线上 https://www.novel-agent.cn。**勿启旧 Consumer 连生产 RabbitMQ。** 详见 `CLAUDE.md`。
