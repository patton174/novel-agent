...
cd /opt/novel-agent
ENV_FILE='novel-agent/agent-document/docs/deploy/docker/.env.worker'
...
for f in novel-agent/agent-document/docs/deploy/docker/nacos-split/*.yaml; do
...
docker run --rm \
  ...
  python:3.11-slim \
  bash -c "pip install -q httpx && python novel-agent/scripts/publish_nacos_config.py"
