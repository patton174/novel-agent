...
#
# 用法：
#   bash novel-agent/agent-document/docs/deploy/scripts/setup-https.sh www.novel-agent.cn your@email.com
#
set -euo pipefail
...
set -euo pipefail
DD="$MW_DIR/novel-agent/agent-document/docs/deploy/docker"
cd "$MW_DIR"
CF=novel-agent/agent-document/docs/deploy/docker/docker-compose.mw.yml
EF=novel-agent/agent-document/docs/deploy/docker/.env.mw
