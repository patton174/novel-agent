...
#
# 本机触发（两台都更新）：
#   bash novel-agent/docs/deploy/scripts/deploy-from-git.sh
#
# 仅更新指定服务：
#   bash novel-agent/docs/deploy/scripts/deploy-from-git.sh gateway mw
#   bash novel-agent/docs/deploy/scripts/deploy-from-git.sh frontend worker
#
# 环境变量：
...
bash novel-agent/docs/deploy/scripts/_server-deploy.sh "\$ROLE" "\$ONLY_SVC"
EOF
}
