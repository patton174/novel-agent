...
#
#   bash novel-agent/docs/deploy/scripts/deploy-phase3-online.sh
#   SKIP_RSYNC=1 bash novel-agent/docs/deploy/scripts/deploy-phase3-online.sh
set -euo pipefail
...
deploy_scp_file "$SCRIPT_DIR/deploy-phase3-remote.sh" "$WORKER_SSH" "$WORKER_DIR/novel-agent/docs/deploy/scripts/deploy-phase3-remote.sh"
deploy_ssh "$WORKER_SSH" "chmod +x '$WORKER_DIR/novel-agent/docs/deploy/scripts/deploy-phase3-remote.sh' && bash '$WORKER_DIR/novel-agent/docs/deploy/scripts/deploy-phase3-remote.sh'"

echo ""
echo "[phase3] 部署完成"
echo "  站点: https://www.novel-agent.cn"
echo "  PyAI 已切换 agent.runtime.mode=queued（Worker 接管 Run）"
echo "  日志: ssh $WORKER_SSH 'cd $WORKER_DIR && docker compose -f novel-agent/docs/deploy/docker/docker-compose.worker.yml logs -f --tail=100 agent-consumer agent-pyai python-ai'"
