...
FAST="$REPO_ROOT/novel-agent/docs/deploy/scripts/deploy-fast.sh"
MVN_IMAGE="${MVN_IMAGE:-maven:3.9-eclipse-temurin-21}"

build_jar_remote() {
  local module="$1"
  local jar="$2"
  local out="$REPO_ROOT/novel-agent/$module/target/$jar"
  mkdir -p "$(dirname "$out")"
...
deploy_frontend() {
  ...
  cd "$REPO_ROOT"
  local cf="novel-agent/docs/deploy/docker/docker-compose.worker.yml"
  local env_rel="novel-agent/docs/deploy/docker/.env.worker"
  COMPOSE="docker compose"
  if ! docker compose version >/dev/null 2>&1; then COMPOSE="docker-compose"; fi
...
deploy_list=()
if [[ "$ROLE" == "mw" ]]; then
  need "novel-agent/agent-gateway/*" && deploy_list+=(gateway)
  need "novel-agent/agent-auth/*" && deploy_list+=(auth)
  if [[ ${#deploy_list[@]} -eq 0 && ${#CHANGED[@]} -gt 0 ]]; then
    need "novel-agent/agent-common/*" && deploy_list+=(gateway auth)
  fi
else
  need "novel-agent/agent-pyai/*" && deploy_list+=(pyai)
  need "novel-agent/agent-content/*" && deploy_list+=(content)
  need "novel-agent/agent-consumer/*" && deploy_list+=(consumer)
