...
#
# 用法（Git Bash）：
#   bash novel-agent/docs/deploy/scripts/deploy-remote.sh
set -euo pipefail
...
COMPOSE_FILE="novel-agent/docs/deploy/docker/docker-compose.yml"
ENV_REL="novel-agent/docs/deploy/docker/.env"
