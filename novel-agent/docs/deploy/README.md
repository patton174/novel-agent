...
| 角色 | 典型职责 | 主要服务 |
|------|----------|----------|
| **MW** | 中间件、入口 Nginx、Gateway、Auth | PostgreSQL、Redis、RabbitMQ、Nacos |
| **Worker** | 业务 Java、Python AI、前端静态 | Content、PyAI、Consumer、前端 Nginx |

域名示例：`https://www.novel-agent.cn`

## 前置条件

1. 服务器已 clone 仓库至 `/opt/novel-agent`（或自定义 `MW_REMOTE_DIR` / `WORKER_REMOTE_DIR`）
2. 配置 split 环境：`novel-agent/docs/deploy/docker/.env.split`（勿提交）
