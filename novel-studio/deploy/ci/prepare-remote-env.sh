...
AUTH_FRONTEND_BASE_URL=\${AUTH_FRONTEND_BASE_URL:-https://novel-agent.cn}
CLIENT_SECURITY_ENABLED=true
CLIENT_SECURITY_AES_REQUIRED=true
CLIENT_SECURITY_ROUTE_OBFUSCATION=true
CLIENT_SECURITY_FIELD_ENCRYPTION=true
CLIENT_SECURITY_ENCRYPT_STREAM=true
...
if [[ -f "\$NEW/.env.mw" ]]; then
  ...
  DOMAIN="\${DOMAIN:-novel-agent.cn}"
  DOMAIN_ALIASES="\${DOMAIN_ALIASES:-www.novel-agent.cn}"
