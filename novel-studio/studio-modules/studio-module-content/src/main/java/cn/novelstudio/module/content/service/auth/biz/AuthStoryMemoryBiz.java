...
public class AuthStoryMemoryBiz extends BaseBiz {
    ...
    public Result<Map<String, Object>> persistNovelColdScopePatch(
        Long userId,
        String novelId,
        cn.novelstudio.module.content.dto.PersistStoryMemoryScopePatchRequest request
    ) {
        storyMemoryService.persistNovelColdScopePatch(
            userKey(userId),
            novelId,
            request.scope(),
            request.itemId(),
        ...
        return ok(Map.of("ok", true, "novel_id", novelId, "scope", request.scope()));
    }
