...
import cn.novelstudio.module.content.dto.agent.AgentCommandDTO;
...
public class AgentRunDispatchListener {
    ...
    public AgentRunDispatchListener(
        ...
        InternalAgentRunBiz internalAgentRunBiz,
    ...
    private JsonNode fetchCommandPayload(String runId, String commandId) throws Exception {
        try {
            ResponseEntity<AgentCommandDTO> response = internalAgentRunBiz.getCommand(runId, commandId);
            AgentCommandDTO cmd = response.getBody();
            if (cmd == null || cmd.getPayloadJson() == null || cmd.getPayloadJson().isBlank()) {
                return null;
            }
            return objectMapper.readTree(cmd.getPayloadJson());
        } catch (Exception ex) {
