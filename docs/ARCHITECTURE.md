...
> 功能验收走 **https://www.novel-agent.cn**；代码变更走 **CI / `deploy-fast.sh`**。
...
详细协议见：`novel-agent/agent-document/docs/specs/2026-05-29-agent-step-orchestration-design.md`。
...
规格：`novel-agent/agent-document/docs/specs/2026-05-30-story-memory-mq-pg.md`。
...
- **Nacos：** 生产服务发现与 Gateway 安全 flag；模板见 `novel-agent/agent-document/docs/deploy/docker/nacos-split/`
...
| 改动范围 | 动作 |
|----------|------|
| `python-ai/` Agent、工具、提示词、爬虫编排 | **部署 Worker python-ai**（CI 或 `workflow_dispatch`） |
| `novel-agent/agent-*` Java | **deploy-fast** 对应服务 或 CI push |
| `frontend/` 仅 TSX/CSS | 本地 HMR；线上走 CI frontend |
| `frontend/` 依赖 / vite.config / security | CI frontend + 常连带 content |
| `agent-gateway` / `agent-auth` 安全 | **MW 同批** gateway + auth |
| 仅文档/测试 | 不必 |

跨 Python + Java + 前端的多文件重构、SSE 协议变更 → **CI 全量或手动多选 deploy**。

详见 `.cursor/rules/deploy-ops.mdc`。
...
| 组件 | 路径 |
|------|------|
| 主编排 loop / prompt | `python-ai/app/crawl_orchestrator/` |
| 子 Agent 工具 | `python-ai/app/crawl_agent/` |
| Content 任务/书库/内部 API | `novel-agent/agent-content/.../crawl/` |
...
|------|------|
| `CLAUDE.md` | 协作规范 |
| `.cursor/rules/project-architecture.mdc` | AI 架构速查（记忆） |
| `.cursor/rules/deploy-ops.mdc` | AI 部署速查（记忆） |
| `novel-agent/agent-document/docs/deploy/README.md` | 生产部署指南 |
...
| `python-ai/app/crawl_orchestrator/loop.py` | 爬虫主编排 |
...
| 文件 | 说明 |
|------|------|
| `novel-agent/agent-document/docs/specs/2026-05-29-agent-step-orchestration-design.md` | Agent Step 编排 |
...
*文档版本：2026-06-07 · 与仓库代码同步维护；AI 持久记忆见 `.cursor/rules/project-architecture.mdc` 与 `deploy-ops.mdc`。*
