...
当前 `novel-agent/pom.xml` 的 `<modules>` 平铺 7 个：`agent-common`(聚合)、`agent-feign`(聚合)、`agent-gateway`、`agent-pyai`、`agent-auth`、`agent-content`、`agent-consumer`。后 5 个业务服务与基础设施模块同级，缺少"服务层"聚合；文档与部署脚本散落在 `novel-agent/agent-document/docs/`，无独立模块语义。
...
novel-agent/
  ...
  agent-document/              # 新增（packaging=pom）
    pom.xml
    docs/                      # ← novel-agent/agent-document/docs/ 全量移入
```
...
**B2.2 父 `novel-agent/pom.xml` 的 `<modules>`**
```xml
<modules>
...
> 部署脚本/Docker 大量**硬编码模块路径**与 `novel-agent/agent-document/docs/` 相对路径，移动后必须逐一同步，否则部署链路断裂。

| # | 文件 | 现状 | 改为 |
|---|------|------|------|
| 1 | `Dockerfile.java` | `COPY novel-agent/agent-auth ...`（5 行）；`COPY /build/novel-agent/${MODULE}/target/${JAR}` | `COPY novel-agent/agent-service/agent-auth ...`；`${MODULE}` 传 `agent-service/agent-auth`，或将 `COPY` 路径改为 `agent-service/${MODULE}` |
...
| 4 | `docs/deploy/scripts/*.sh` | 内含 `novel-agent/agent-document/docs/deploy/...` 字面量 | → `novel-agent/agent-document/docs/deploy/...` |
...
rg "novel-agent/agent-document/docs/" -l
...
- **B4-彻底（高风险）**：物理 `git mv novel-agent/docs novel-agent/agent-document/docs`，按 B.3 #4#5 全量改脚本路径与上溯层级，逐脚本验证。需一次性完成并实跑一次部署冒烟。
...
docker build --build-arg MODULE=agent-service/agent-auth --build-arg JAR=agent-auth-1.0.0-SNAPSHOT.jar \
  -f novel-agent/agent-document/docs/deploy/docker/Dockerfile.java .   # 路径按最终方案调整
```
