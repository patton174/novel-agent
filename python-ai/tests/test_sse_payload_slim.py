...
from app.agent_step.events import extract_memory_read_labels, _tool_completed_payload
from app.agent_step.schemas import DisplayPayload
...
def test_tool_completed_memory_read_omits_full_output():
    ...
    payload = _tool_completed_payload(
        ...
        content=content,
        failed=False,
        display=DisplayPayload(type="tool", tool="memory_read", content=content),
    )
...
def test_tool_completed_memory_update_uses_action_label():
    ...
    payload = _tool_completed_payload(
        ...
        content=content,
        failed=False,
        display=DisplayPayload(type="tool", tool="memory_update", content=content),
    )
