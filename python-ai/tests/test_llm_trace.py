...
from app.agent.harness.llm_trace import log_llm_exchange, trace_enabled


def test_log_llm_exchange_writes_utf8(tmp_path, monkeypatch):
    trace_file = tmp_path / "trace.log"
    monkeypatch.setattr("app.agent.harness.llm_trace.settings.agent_llm_trace", True)
    monkeypatch.setattr("app.agent.harness.llm_trace.settings.agent_llm_trace_file", str(trace_file))
    monkeypatch.setattr("app.agent.harness.llm_trace._FILE_LOGGER", None)
