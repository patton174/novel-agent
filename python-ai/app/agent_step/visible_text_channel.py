...
def visible_text_channel_prompt_block() -> str:
    return """## 可见文本分区（UI 编排区 vs 正文区）

同轮 assistant **可见文本**（非 reasoning）按前缀分流到 UI：
- **编排区**：**仅**展示以 `[编排]`（或别名）开头的文字；**无前缀的可见文字不会出现在编排区**
- **正文区**：**仅**展示 `[交付]`（或别名）之后的文字

是否写编排说明**由你自行决定**——可以不写、直接调工具；若希望用户看到进度，再在开头加 `[编排]`（宜短，≤1–2 句）。

若需写入编排区或正文区，请在对应片段**最开头**写固定前缀，前缀后**空一行**再写正文：
- 编排说明（可选）→ `[编排]`
- 最终交付（终轮必须）→ `[交付]`
...
将先 Glob 列章，再 Read 最新章前 50 行。
```
示例（交付，整轮结束且无 tool_use 时必须）：
```
[交付]
...
- **有 tool_use 的轮次**：编排说明**完全可选**；**只有**带 `[编排]` 前缀的句子会进入编排区
- **无 tool_use 的最后一轮**：必须含 `[交付]` 前缀（可在全文任意位置）；**仅 `[交付]` 之后**的内容进入正文区
- 允许别名（大小写不敏感）：`[NARR]` / `[MSG]` / `NARR:` / `MSG:` / `编排:` / `交付:`
- **无前缀的可见文字服务端一律不展示**（勿把步骤汇报写在无前缀正文里）

## 编排 vs 交付（硬性规则）

- **`[编排]` 若写则宜短**：**最多 1–2 句**或一行 `- 将 Read …`；不写也完全允许
- **禁止**在无前缀可见文字或 `[编排]` 里写完整自检报告（那些属于 `[交付]`）
- **`[交付]` 只在整轮 run 结束且无 tool_use 时使用**：任务已全部完成后的用户可见总结
- **同一轮有 tool_use 时禁止 `[交付]`**（有工具就不是交付轮）
- **AskUser / 用户确认（硬性）**：
