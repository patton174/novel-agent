...
def build_crawl_run_context(ctx: CrawlAgentContext) -> str:
    ...
    parts.extend(
        [
            ...
            "- 从网页批量抓书：MapLinks 或 FetchPage → QueueChapters → InitNovel → SaveQueuedChapters",
...
def _run_context_content(ctx: CrawlAgentContext) -> str:
    return f"{RUN_CONTEXT_MARKER}\n{build_crawl_run_context(ctx)}"


def _is_run_context_message(msg: object) -> bool:
    return isinstance(msg, HumanMessage) and isinstance(msg.content, str) and msg.content.startswith(
        RUN_CONTEXT_MARKER
    )


def refresh_crawl_run_context(messages: list, ctx: CrawlAgentContext) -> None:
    block = _run_context_content(ctx)
    for i, msg in enumerate(messages):
        if _is_run_context_message(msg):
            messages[i] = HumanMessage(content=block)
            return
    insert_at = 1 if messages and isinstance(messages[0], SystemMessage) else 0
    messages.insert(insert_at, HumanMessage(content=block))
