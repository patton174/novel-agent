...
from app.crawl.orchestrator.context_builder import build_cycle_context, format_cycle_context
from app.crawl.orchestrator.tools import ORCHESTRATOR_TOOLS, run_orchestrator_tool
...
## 派发规则（严格遵守）
1. 同时占用槽位（RUNNING+PAUSED）最多 3；**PAUSED 仍占槽**
2. **禁止**对同一 sourceUrl 创建第二个活跃任务（含同一轮连续 Create）
3. **禁止**无 sub_goal 的盲目 CreateCrawlJob；示例：
   - 详情页：「抓取《XXX》全部章节入库」
   - 封面：「为 catalogNovelId=xxx 提取封面 UpdateCoverUrl」（须传 catalog_novel_id）
4. **「爬 N 本书」**拆法：
   - 最多 1 个发现子任务（首页→分类/list 页→拿到详情 URL 列表）
   - 发现完成后 **CancelCrawlJob** 释放发现任务槽位
   - 每本书单独 Create：`sourceUrl=详情页`，sub_goal=「抓取《书名》全部章节入库」
   - **禁止**同一 sourceUrl 派两个任务（例如两个 list/wangyou/）
5. 重复/失败/占槽无用的 PAUSED 任务 → **CancelCrawlJob** 释放槽位（不要反复 Stop）
6. 槽位满 → 先 Cancel 重复 PAUSED，再 Create；仍满则 Sleep
7. 总目标完成 → CompleteGoal；无事可做 → Sleep

## 工具
GetOrchestratorState / ListIncompleteCatalog / ListCrawlJobs / GetRunningJobCount /
CreateCrawlJob / StopCrawlJob / CancelCrawlJob / GetCrawlJobStatus / Sleep / CompleteGoal
