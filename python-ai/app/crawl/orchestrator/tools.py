...
from app.crawl.orchestrator.scheduler import get_orchestrator_scheduler, normalize_source_url
    ...
    *,
    ...
    try:
        ...
        if name == "CreateCrawlJob":
            ...
            scheduler = get_orchestrator_scheduler()
            cycle_urls = set(cycle_ctx.get("activeSourceUrls") or []) if cycle_ctx else None
            ok, reason = await scheduler.can_create_job(
                client,
                source_url,
                cycle_active_urls=cycle_urls,
            )
            if not ok:
                return json.dumps({"ok": False, "error": reason})
            norm = normalize_source_url(source_url)
            scheduler.register(source_url)
