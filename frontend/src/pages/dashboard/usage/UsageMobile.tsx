    ...
    <article className="rounded-xl border border-border bg-surface p-3.5 text-sm shadow-soft">
      <div className="flex items-start justify-between gap-2">
        ...
        <span className="shrink-0 tabular-nums text-xs text-muted-foreground">
          {ev.totalTokens.toLocaleString(dateLocale)} {t('dashboard:billing.tokenAbbrev')}
        </span>
      </div>
      <dl className="mt-2 space-y-1 text-xs text-muted-foreground">
        ...
        <div className="flex justify-between gap-2">
          ...
          <dd className="text-right">{new Date(ev.createdAt).toLocaleString(dateLocale)}</dd>
        </div>
      </dl>
...
export function UsageMobile() {
  ...
  return (
    <AppPageStack className="gap-8">
      ...
      <AppShellCard>
        <AppShellCardHeader title={t('dashboard:billing.usageTitle')} />
        <AppShellCardBody>
