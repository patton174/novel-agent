...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (remainingEmailCooldownSec(trimmed, 'forgot') > 0 || submitting || captchaOpen) {
  ...
  const handleCaptchaVerified = async (captchaToken: string) => {
    ...
    try {
      ...
      await requestPasswordReset(trimmed, captchaToken)

      writeEmailCooldown(trimmed, 60, 'forgot')
