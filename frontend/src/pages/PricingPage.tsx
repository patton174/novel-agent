...
export default function PricingPage() {
  ...
  }, [searchParams, plans, isLoggedIn, navigate])
  ...
  const renderTierCta = (tier: PlanPublic) => {
    ...
    if (tier.priceCents != null && tier.priceCents > 0) {
      if (isLoggedIn) {
        return (
          <button type="button" className={ctaClass} onClick={() => selectCheckout(tier.code)}>
      ...
      return (
        <Link
          to={buildLoginHref({ returnPath: `/checkout?plan=${encodeURIComponent(tier.code)}` })}
          className={ctaClass}
        >
          {t('dashboard:billing.payNow')}
        </Link>
